...
export async function POST(request: NextRequest) {
  try {
    ...
    const documentType = searchParams.get("document_type") || formData.get("document_type") as string;
    
    console.log('🔍 Next.js API - Document Type:', documentType);
    console.log('🔍 Next.js API - URL:', request.url);
    console.log('🔍 Next.js API - Search Params:', Object.fromEntries(searchParams.entries()));
    ...
    // Validate file type
    if (!file.type.includes('pdf') && !file.type.includes('image') && !file.name.toLowerCase().endsWith('.pdf')) {
    ...
    if (!documentType) {
      ...
      return NextResponse.json(
        { success: false, message: "No document_type provided", status: "error" },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      console.log('🌐 Next.js API - AI service reachability test:', testResponse.status);
    } catch (testError) {
    ...
    for (const endpoint of possibleEndpoints) {
      try {
        console.log(`🔄 Next.js API - Trying endpoint: https://ai.pianificosuite.it${endpoint}`);
        ...
        console.log(`📊 Next.js API - Response status for ${endpoint}:`, aiResponse.status);
        
        // Log response headers for debugging
        console.log(`📊 Next.js API - Response headers:`, Object.fromEntries(aiResponse.headers.entries()));
        ...
        if (aiResponse.ok) {
          ...
          try {
            ...
            console.log(`❌ Next.js API - Failed with endpoint ${endpoint}, status: ${aiResponse.status}, response: ${errorText}`);
            lastError = `Status: ${aiResponse.status}, Response: ${errorText}`;
          } catch (textError) {
            console.log(`❌ Next.js API - Failed with endpoint ${endpoint}, status: ${aiResponse.status}`);
        ...
        if (error instanceof Error && error.name === "AbortError") {
          ...
          console.log(`⚠️ Next.js API - Other error for endpoint ${endpoint}:`, error);
