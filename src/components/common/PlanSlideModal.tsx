  ...
  if (isLoading || !plan) {
    return (
      <>
        {/* Backdrop */}
        <div
          className={`fixed inset-0 bg-gray-800 z-50 transition-all duration-300 ${
  ...
  return (
    <>
      ...
      >
        ...
        <button
          onClick={prevSlide}
          className={`absolute left-1 sm:left-2 md:left-4 top-1/2 transform -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 flex items-center justify-center z-20 rounded-full transition-colors cursor-pointer bg-black/20 hover:bg-black/30 ${
            activeButton === "left" ? "bg-primary/40" : ""
          }`}
        >
          <FaCaretLeft
            className={`w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 transition-colors ${
              activeButton === "left" ? "text-primary" : "text-white"
            }`}
          />
        </button>
        ...
        <button
          onClick={nextSlide}
          className={`absolute right-1 sm:right-2 md:right-4 top-1/2 transform -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 flex items-center justify-center z-20 rounded-full transition-colors cursor-pointer bg-black/20 hover:bg-black/30 ${
            activeButton === "right" ? "bg-primary/40" : ""
          }`}
        >
          <FaCaretRight
            className={`w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 transition-colors ${
              activeButton === "right" ? "text-primary" : "text-white"
            }`}
          />
        </button>
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              {currentSlideData.isFinancial ? (
                <div className="space-y-6">
                  ...
                  {currentSlideData.title === "Key Ratios" &&
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-6 border border-indigo-200">
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          {(() => {
                            const totalAssets = currentSlideData.content.reduce(
                              (sum: number, item: any) =>
                                sum + (item.assets || 0),
                              0
                            );
                            const totalLiabilities =
                              currentSlideData.content.reduce(
                                (sum: number, item: any) =>
                                  sum + (item.liabilities || 0),
                                0
                              );
                            const totalEquity = currentSlideData.content.reduce(
                              (sum: number, item: any) =>
                                sum + (item.equity || 0),
                              0
                            );
                            ...
                            const totalValue =
                              avgAssets + avgLiabilities + avgEquity;
                            if (totalValue === 0) return null;

                            const assetsPercentage =
                              (avgAssets / totalValue) * 100;
                            const liabilitiesPercentage =
                              (avgLiabilities / totalValue) * 100;
                            const equityPercentage =
                              (avgEquity / totalValue) * 100;

                            // Create pie chart using CSS conic-gradient
                            const conicGradient = `conic-gradient(
                              from 0deg,
                              #10b981 0deg ${assetsPercentage * 3.6}deg,
                              #ef4444 ${assetsPercentage * 3.6}deg ${
                              (assetsPercentage + liabilitiesPercentage) * 3.6
                            }deg,
                              #3b82f6 ${
                                (assetsPercentage + liabilitiesPercentage) * 3.6
                              }deg 360deg
